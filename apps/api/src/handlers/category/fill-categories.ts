...
export const fillCategoriesWithSlugAndWomenImage = async () => {
  try {
    console.log("Starting to fill categories with slug and womenImage...");

    for (const [categoryName, categoryData] of Object.entries(
      allCategoriesData
    )) {
      // Extract slug from the category name (convert to kebab-case)
      const slug = categoryName.toLowerCase().replace(/\s+/g, "-");

      // Use the image as womenImage
      const womenImage = categoryData.image;

      console.log(`Processing category: ${categoryName} with slug: ${slug}`);

      // Find existing category by name and update it
    ...
    console.log("✅ Finished filling categories with slug and womenImage!");
  } catch (error) {
