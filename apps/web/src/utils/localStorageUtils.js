export function addToLocalStorage(key, value, limit = 200) {
    ...
    if (isAlreadyStored) {
        storedTermsArray = storedTermsArray.filter(itemSearchString => itemSearchString !== value);
        storedTermsArray.push(value);
    }
    else {
        storedTermsArray.push(value);
        if (storedTermsArray.length > limit) {
            storedTermsArray = storedTermsArray.slice(storedTermsArray.length - limit);
        }
    }
...
export function getFromLocalStorage(key) {
    let storedTermsArray = JSON.parse(localStorage.getItem(key) || '[]').reverse()
