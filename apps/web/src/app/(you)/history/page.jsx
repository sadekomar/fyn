...
export default function HistoryPage() {
    ...
    useEffect(() => {
        ...
        async function fetchData(recentlyViewedString) {
            ...
            try {
                if (recentlyViewed != 0) {
                    const response = await fetch(`${IPAddress}/ids?ids=${recentlyViewedString}`, {
...
function ClearHistoryButton({ setProducts, setIsEmpty }) {
    ...
    return <AlertDialog.Root>
        <AlertDialog.Trigger>
            <Button variant='soft' className='alert-dialog-button'>Clear History</Button>
        </AlertDialog.Trigger>
        <AlertDialog.Content className='alert-dialog-content' style={{ maxWidth: '450px' }}>
            <AlertDialog.Title>Clear History</AlertDialog.Title>
            <AlertDialog.Description>
                Are you sure? Your browsing history will be cleared, and you won't be able to restore it.
            </AlertDialog.Description>
            ...
            <Flex gap="2" mt="4" justify="end">
                ...
                <AlertDialog.Action>
                    <Button variant="solid" px={2} onClick={clearHistory} className='alert-dialog-button' color="red">
                        Clear History
                    </Button>
                </AlertDialog.Action>
            </Flex>
