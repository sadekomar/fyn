...
export function AddToCart({ id, className }) {
    ...
    return <>
        <Toast.Provider swipeDirection="right">
            ...
            <button className={`cart-button ${isFilled ? 'cart-button-added' : ''}`} onClick={() => {
            ...
            <Toast.Root className="ToastRoot" open={open} onOpenChange={setOpen}>
                <Toast.Title className="ToastTitle">
                    {isFilled ? 'Added item to your cart' : 'Removed item from your cart'}
                </Toast.Title>
                <Toast.Action className="ToastAction" asChild altText="view cart">
                    <Link className='view-cart-button' href={'/cart'}>View Cart</Link>
                </Toast.Action>
            </Toast.Root>
        </Toast.Provider >
    </>;
