...
export async function BrandScroller({ brand, title }) {
    ...
    let response = await fetch((`${IPAddress}/search?brand=${brand}&limit=20&sort_by=date-descending`))
    await new Promise(resolve => setTimeout(resolve, 3000))
    let data = await response.json()

    return <>
        <div className='h-scroller-title'>
            <h3>{title}<Link href={`/brands/${brand}`} className='brand-link'>{brand}</Link></h3>
